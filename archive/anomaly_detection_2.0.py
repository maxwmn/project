...
Same pipeline as anomaly_detection.py (one shared model, using SCADA sensor
readings to predict anomalies), but using gradient boosting (XGBoost)
instead of a Random Forest, to compare the two approaches. Requires the
...
def main():
    ...
    # Step 5: compute a class-imbalance weight, since anomalies are much rarer
    # than normal rows. XGBoost has no class_weight="balanced" option; the
    # standard equivalent is scale_pos_weight = (number of negatives) /
    # (number of positives) in the training data.
    n_pos = int(y_train.sum())
    n_neg = len(y_train) - n_pos
    scale_pos_weight = n_neg / n_pos

    # Step 6: configure and train one XGBoost classifier on the pooled training data.
    model = XGBClassifier(
    ...
    # Step 7: predict on the held-out turbines and print overall accuracy/precision/recall/f1.
    y_pred = model.predict(X_test)
    overall = evaluate(y_test, y_pred)
    print("\nOverall performance (turbine-holdout, event-window labels, trend features, XGBoost):")
    for key, value in overall.items():
        print(f"  {key}: {value}")

    # Step 8: break the same model's predictions down per held-out turbine.
    ...
    # Step 9: save the per-turbine results to a CSV and print them.
    ...
    # Step 10: show which sensor features mattered most to the model's decisions.
    importances = pd.Series(model.feature_importances_, index=feature_cols)
    top_features = importances.sort_values(ascending=False).head(15)
    print("\nTop 15 features by importance:")
    print(top_features)

    # Step 11: save the overall metrics to a CSV as well.
