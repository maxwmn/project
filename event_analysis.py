...
PRE_EVENT_WINDOW = pd.Timedelta(hours=24 * 2)  # 2 days
...
def load_event_info(path):
    # Loads event_info.csv and converts event_start/event_end to datetime,
    # so time-based comparisons (e.g. "48 hours before") are possible
...
def load_all_cleaned_files(folder):
    ...
    for file_path in files:
        ...
        with open(file_path, "r") as f:
            first_line = f.readline()
        sep = ";" if ";" in first_line else ","
        # Cleaned files use comma as the decimal separator (set when saving in the cleaning script)
...
def get_pre_event_data(df, event_start, window=PRE_EVENT_WINDOW):
    # Returns only the rows within the defined window before event_start
    event_start = pd.to_datetime(event_start)
    return df[(df[TIME_COL] >= event_start - window) & (df[TIME_COL] < event_start)]
 
 
def get_event_data(df, event_start, event_end):
    """Returns only the rows that fall within the event's own duration (event_start to event_end)."""
    event_start = pd.to_datetime(event_start)
    event_end = pd.to_datetime(event_end) if pd.notna(event_end) else event_start
    return df[(df[TIME_COL] >= event_start) & (df[TIME_COL] <= event_end)]
 
 
def status_distribution(df):
    """Share (0.0-1.0) of each status_type_id value in a DataFrame."""
    return df[STATUS_COL].value_counts(normalize=True).sort_index()
...
# =============================================================
# Per-turbine analysis (one overview plot + one status table per turbine)
# =============================================================
 
def plot_turbine_overview(asset_id, anomaly_entries, normal_entries, output_folder,
                           plot_window_before=pd.Timedelta(days=100),
    ...
    # Precompute hourly data once per file, and collect all status values
    # so every subplot uses the same color for the same status
    hourly_cache = {}
    all_statuses = set()
    ...
    # One subplot per anomaly event, showing the status timeline as
    # colored blocks with the event duration highlighted
    ...
    # Last subplot: normal-condition reference, using the same window length
    ax_normal = axes[-1]
...
def build_turbine_status_table(asset_id, anomaly_entries, normal_df, output_folder):
    ...
    table.to_csv(out_path, sep=";", decimal=",")
    return out_path
 
 
def analyze_turbine(asset_id, entries, output_folder):
    # Splits a turbine's files into normal vs. anomaly, then creates its
    # combined overview plot and status table (skipped if either group is missing)
...
# =============================================================
# Summary across all turbines and events
# =============================================================
 
def build_summary_table(groups, output_folder):
    """
    Builds ONE table across ALL turbines and events. Each row represents
    one status_type_id value for one specific event, showing its share
    during the normal baseline, the pre-event window, and the event
    itself - plus how much each differs from normal. Turbines stay
    clearly separated via the asset_id column (and the sort order).
    """
    rows = []
    ...
    for asset_id, entries in groups.items():
        ...
        normal_df = pd.concat([e["df"] for e in normal_entries], ignore_index=True)
        normal_dist = status_distribution(normal_df)
 
        for entry in anomaly_entries:
            pre_df = get_pre_event_data(entry["df"], entry["event_start"])
            event_df = get_event_data(entry["df"], entry["event_start"], entry["event_end"])
 
            pre_dist = status_distribution(pre_df) if not pre_df.empty else pd.Series(dtype=float)
            event_dist = status_distribution(event_df) if not event_df.empty else pd.Series(dtype=float)
 
            all_statuses = sorted(set(pre_dist.index) | set(event_dist.index) | set(normal_dist.index))
 
            for status in all_statuses:
                normal_share = normal_dist.get(status, 0)
                pre_share = pre_dist.get(status, 0)
                event_share = event_dist.get(status, 0)
                ...
                rows.append({
                    ...
                    "pre_event_share": pre_share,
                    "event_share": event_share,
                    "diff_pre_event_vs_normal": pre_share - normal_share,
                    "diff_event_vs_normal": event_share - normal_share,
                })
 
    summary_df = pd.DataFrame(rows).sort_values(["asset_id", "event_id", "status_type_id"])
 
    out_path = os.path.join(output_folder, "summary_all_events.csv")
    summary_df.to_csv(out_path, sep=";", index=False, decimal=",")
    return summary_df, out_path
 
 
def plot_summary_overview_averaged(summary_df, output_folder):
    """
    Creates ONE combined figure with one bar-chart subplot per turbine,
    showing the AVERAGE share of each status_type_id across all of that
    turbine's events - normal vs. pre-event vs. event.
    """
    asset_ids = summary_df["asset_id"].unique()
    n = len(asset_ids)
    ...
    for ax, asset_id in zip(axes, asset_ids):
        sub = summary_df[summary_df["asset_id"] == asset_id]
        agg = sub.groupby("status_type_id")[["normal_share", "pre_event_share", "event_share"]].mean()
        agg.plot(kind="bar", ax=ax)
        ax.set_title(f"Turbine {asset_id}")
        ax.set_xlabel("status_type_id")
        ax.set_ylabel("Share")
        ax.legend(["Normal", "Pre-event", "Event"])
 
    fig.tight_layout()
    out_path = os.path.join(output_folder, "summary_bars_averaged.png")
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    return out_path
 
 
def plot_summary_overview_per_event(summary_df, output_folder):
    """
    Creates ONE figure PER TURBINE. Within each figure, one subplot
    per anomaly event, comparing normal vs. pre-event vs. event shares
    for each individual event (no averaging).
    """
    output_paths = []
    asset_ids = summary_df["asset_id"].unique()
 
    for asset_id in asset_ids:
        sub_asset = summary_df[summary_df["asset_id"] == asset_id]
        event_ids = sub_asset["event_id"].unique()
        n = len(event_ids)
        ...
        for ax, event_id in zip(axes, event_ids):
            sub_event = sub_asset[sub_asset["event_id"] == event_id]
            data = sub_event.set_index("status_type_id")[["normal_share", "pre_event_share", "event_share"]]
        ...
        fig.suptitle(f"Turbine {asset_id} — Status shares per event", y=1.0)
        fig.tight_layout()
 
        out_path = os.path.join(output_folder, f"summary_bars_asset{asset_id}.png")
        fig.savefig(out_path, dpi=150, bbox_inches="tight")
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    # Per-turbine outputs: one overview plot + one status table per turbine
    for asset_id, entries in groups.items():
    ...
    # Combined summary across all turbines and events
    summary_df, summary_path = build_summary_table(groups, output_folder)
    averaged_plot_path = plot_summary_overview_averaged(summary_df, output_folder)
    per_event_plot_paths = plot_summary_overview_per_event(summary_df, output_folder)
 
    print(f"\nDone. Turbines processed: {processed}, skipped: {skipped}")
    print(f"Output saved in: {output_folder}")
    print(f"Summary table: {os.path.basename(summary_path)}")
    print(f"Averaged summary plot: {os.path.basename(averaged_plot_path)}")
    print(f"Per-event summary plots: {len(per_event_plot_paths)} files (one per turbine)")
